...
publish: the docx cells that were just pushed to the CMS get recolored so
the docx stays in sync with reality.

Usage:
    python recolor_docx_cells.py <docx_path> <cell_index> [<cell_index> ...]
...
def recolor(docx_path: Path, cell_indices: list[int]) -> dict:
    ...
    targets = sorted(set(cell_indices), reverse=True)
    changed: list[dict] = []
    skipped: list[dict] = []
    ...
    for idx in targets:
        ...
        # Peek at current fill without mutating yet, so we can decide to skip
        # cells whose fill is already a real (non-white) color.
        ...
        new_cell, previous = _rewrite_cell(cell_bytes, PUBLISHED_BY_AI_FILL)
        xml = xml[:start] + new_cell + xml[end:]
        changed.append({
            "cell_index": idx,
            "old_fill": previous,
    ...
    changed.sort(key=lambda r: cell_indices.index(r["cell_index"]))
    ...
    return {"changed": changed, "skipped": skipped, "total_targeted": len(cell_indices)}
...
def main() -> int:
    ...
    if len(sys.argv) < 3:
        sys.stderr.write(
            f"usage: {sys.argv[0]} <docx_path> <cell_index> [<cell_index> ...]\n"
        )
        return 2
    ...
    try:
        ...
        sys.stderr.write(f"cell_index must be an integer: {exc}\n")
        return 2

    result = recolor(docx_path, indices)
